...
20:40 check: CG-343 PR237 mergedc1f75cb; installed pin remains8842552 until a drained update. CG-322 base-broken stop was SIGTERM, not proof of source failure. Fresh detached c1f75cb passed its specific failing rebase test; branch failed it. Added concrete revision evidence and resumed supported state. Preserve work. Web .1.28s, VM available~4.8GiB/tmp43%, no OOM; workers337/338/293 active and358 awaiting revisions at observation. CG-329 safety hold remains until338.

## REOPENED incident override around21:04 UTC

Supersedes earlier closed/normal statements: Now2 timeout recurred under memory.high pressure and overlapping suites. Admission PAUSED, temporary max_parallel1, review1, all resource caps retained. CG-326 preempted and ready with worktree/transcript and operator-test-tmp/CG326-recurrence.patch preserved. CG-358/323 were still active; verify. Now2 recovered to1.12s after containment, not enough to close. Five-minute heartbeat reinstated. Prioritize CG-338 resource admission and CG-358 recovery controls; preserve CG-329 draft safety hold. After verified recurrence recovery add retro/prevention addendum; do not restore3 merely because prior observation passed.

21:13 priority update: CG-338 PR238 merged6dd3ae49f448f088f8aef10aceb331a68620425c; all correctness criteria/reviewer66 focused tests and current-head CI passed. Operator corrected permanent description and declined history-only rewrite as an incident blocker. Task done; NOT yet installed/enabled. CG-358 worker and CG-323 check were active, so retain pause/one-worker containment and wait for accounted drain before install/restart. On deployment configure resources.max_parallel=1 (all local modes combined), min_memory_available_mb=1536, min_temp_free_mb=1024; retain OS caps. Verify effective gate with controlled work before releasing CG-329 safety hold or increasing concurrency. Do not infer merged means protected. Now2 .67s, service memory~1.22GiB at21:11; recurrence observation still open.

Owner-requested serial tmpfs/ext4 test completed: identical full suite atc1f75cb under2CPU/3Ghigh/4Gmax, tmpfs163s/969MiBpeak versus ext4222s/1523MiBpeak. I/O stalls .19% vs11.47%, negligible CPU throttling and no memory pressure in isolation. Ext4 archive-restore index failure reproduced; CG-360 priority0 filed. Do not use production archive/restore until repaired. Live temp symlink unchanged. Report and raw summaries: context-garden/docs/incidents/test-temp-ab/README.md. Both experiment services finished; safe admission-fix deployment remains next after fresh live-run accounting.
