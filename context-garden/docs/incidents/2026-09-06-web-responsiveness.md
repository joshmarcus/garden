...
21:11–21:13: Inbox .74s/Now2 .67s, service memory1.22GiB with prior swap retained. CG-338 latest review marked all four criteria met, only description/history wording remained; operator rewrote permanent description and declined shared-history rewrite as a recovery blocker. PR238 merged6dd3ae49, task done. Deployment awaits active CG-358 work/CG-323 check drain; do not change live config underneath their fences. Proposed initial shared resources.max_parallel1, minimum available memory1536MiB/temp1024MiB plus unchanged OS caps. Gate must be installed and verified before closure or restoring higher concurrency.


## Shared admission deployment and remaining enforcement gap, 2026-09-06 21:43 UTC

Installed CI-verified main6dd3ae49 at a drained tick boundary around21:37, retaining pause and OS caps. Enabled combined resources.max_parallel1, available-memory threshold1536MiB and temp-free threshold1024MiB. Pin installation peak152MiB; existing service restarted as PID1840342. Initial HTTP journeys .25-.68s. Dispatched only CG-361 at21:40:06; client timed out, but existing run/PID1841626 and fresh stdout confirmed successful startup without retry. Effective gate active1/limit1, free slots0. Now2 .876s under the worker, memory333MiB, swap0 and no new memory.high/OOM. Windows browser Now2 -> Open run -> Board worked. Continued loaded observation is required; incident remains open.

Source inspection confirms CG-338 does not bound multiple tests inside one run or reserve a separate web budget. Priority0 CG-361 is the concrete follow-up and active controlled repair. CG-354 focused testing approved at priority1; full CI remains. A/B findings and CG-360 archive hold remain authoritative. This supervised deployment does not count as unattended stabilization.
