...
Usage: operator_spend.py [--session <id>] [--project <dir>]  (defaults: newest session)."""
...
rec = {"at": time.strftime("%Y-%m-%dT%H:%M:%S+00:00", time.gmtime()), "session": sid, "first_turn": first, "last_turn": last, "turns": turns, "models": models, "tokens": tot, "list_price_usd": round(cost, 2), "avg_context": int(tot["cache_read"] / max(1, turns))}
